...
void CommonCLI::loadPrefsInt(FILESYSTEM* fs, const char* filename) {
  ...
  if (file) {
    ...
    file.read((uint8_t *)&_prefs->direct_retry_attempts, sizeof(_prefs->direct_retry_attempts));    // 290
    file.read((uint8_t *)&_prefs->direct_retry_base_ms, sizeof(_prefs->direct_retry_base_ms));      // 291
    file.read((uint8_t *)&_prefs->direct_retry_timing_magic[0], sizeof(_prefs->direct_retry_timing_magic)); // 293
    // next: 295
...
void CommonCLI::savePrefs(FILESYSTEM* fs) {
  ...
  if (file) {
    ...
    file.write((uint8_t *)&_prefs->direct_retry_attempts, sizeof(_prefs->direct_retry_attempts));   // 290
    file.write((uint8_t *)&_prefs->direct_retry_base_ms, sizeof(_prefs->direct_retry_base_ms));     // 291
    uint8_t retry_timing_magic[2] = { DIRECT_RETRY_TIMING_MAGIC_0, DIRECT_RETRY_TIMING_MAGIC_1 };
    file.write(retry_timing_magic, sizeof(retry_timing_magic));                                     // 293
    // next: 295
