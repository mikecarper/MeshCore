...
// In the legacy profile, `frag_off` is the byte offset of `data` within block `block_idx` (global position =
// block_idx*block_size + frag_off), so a fragment is self-placing when received from the selected source.
// In the negotiated v2 profile its high bit is set and the field packs the per-block wire encoding, fragment
// index, and total encoded size; data[] starts with SHA-256:4 of the complete encoded representation, followed
// by up to 171 bytes. Repeating that id in every fragment prevents different seeders' valid-but-different
// DEFLATE streams from being mixed. Keeping the existing OTA_DATA type means deployed opaque relays retain the
// same transfer priority. Old endpoints reject the tagged offset and new endpoints only request it explicitly.
struct DataMsg {
  uint8_t  manifest_id[4];
  uint16_t block_idx;
  uint16_t frag_off;
