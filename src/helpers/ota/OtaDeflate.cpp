...
void OtaTransportInflateReceiver::save_state(State& state) const {
  memcpy(state.mid, _mid, sizeof(state.mid));
  memcpy(state.stream_id, _stream_id, sizeof(state.stream_id));
  state.block = _block;
  state.encoded_len = _encoded_len;
  state.mask = _mask;
  state.need = _need;
  state.partial_retries = _partial_retries;
  state.active = _active;
  state.request_sent = _request_sent;
  state.complete = _complete;
  state.deflated = _deflated;
  state.allow_deflate = _allow_deflate;
  state.legacy_session = _legacy_session;
}

void OtaTransportInflateReceiver::restore_state(const State& state) {
  memcpy(_mid, state.mid, sizeof(_mid));
  memcpy(_stream_id, state.stream_id, sizeof(_stream_id));
  _block = state.block;
  _encoded_len = state.encoded_len;
  _mask = state.mask;
  _need = state.need;
  _partial_retries = state.partial_retries;
  _active = state.active;
  _request_sent = state.request_sent;
  _complete = state.complete;
  _deflated = state.deflated;
  _allow_deflate = state.allow_deflate;
  _legacy_session = state.legacy_session;
}
...
bool OtaTransportInflateReceiver::send_trampoline(void* ctx, const uint8_t* msg,
                                                  uint16_t len, bool flood) {
  return ctx && static_cast<OtaTransportInflateReceiver*>(ctx)->send(msg, len, flood);
}

bool OtaTransportInflateReceiver::send(const uint8_t* msg, uint16_t len, bool flood) {
  if (!_send || !msg || len == 0) return false;

  const uint8_t type = ota_msg_type(msg, len);
  if (type != OTA_GET_MANIFEST && type != OTA_REQ) {
    return _send(_send_ctx, msg, len, flood);               // preserve ordinary DATA/PROOF backpressure
  }

  // OtaSend is an admission callback. Keep negotiation state transactional with the packet: if the
  // underlying queue rejects it, OtaManager will retry and the shim must make the same wire decision.
  State prior;
  save_state(prior);
  ...
  if (type == OTA_GET_MANIFEST) reset_session();

  ReqMsg request;
  // The v2 marker belongs to the historical one-row request shape. Never truncate a future/misconfigured
  // request window while rewriting it, even though the compile-time pipeline guard already keeps ours at 9 bytes.
  if (type == OTA_REQ && len == 9 && decode_req(msg, len, request)) {
    ...
    if (!_legacy_session) {
      ...
      if (!_legacy_session) {
        ...
        if (wire_len != 0) {
          _request_sent = true;
          const bool accepted = _send(_send_ctx, wire, wire_len, flood);
          if (!accepted) restore_state(prior);
          return accepted;
        }
      }
    }
  ...
  const bool accepted = _send(_send_ctx, msg, len, flood);
  if (!accepted) restore_state(prior);
  return accepted;
}

bool OtaTransportInflateReceiver::on_message(const uint8_t* msg, uint16_t len) {
  if (!_manager || !msg || len == 0) return false;

  DataMsg data;
  if (ota_msg_type(msg, len) == OTA_DATA && decode_data(msg, len, data)) {
    if ((data.frag_off & OTA_DATA_V2_MARK) != 0) {
      // Matching transfer DATA is terminal even when malformed, just like OtaManager's canonical path:
      // consume it locally instead of echoing hostile or broken fragments. Unsolicited v2 DATA remains
      // relayable, so an intermediate bridge does not swallow another node's transfer.
      const bool terminal = _manager->requestedBlockLength(data.manifest_id, data.block_idx) != 0;
      (void)handle_v2_data(msg, len);
      return terminal;
    }
  ...
  return _manager->on_message(msg, len);
}
...
bool OtaTransportInflateReceiver::handle_v2_data(const uint8_t* msg, uint16_t len) {
  ...
  for (uint16_t off = 0; off < block_len; off = (uint16_t)(off + OTA_FRAG_DATA)) {
    ...
    if (!_manager->on_message(wire, wire_len)) {
      reset_representation();
      return false;
    }
  }
...
#endif // (ENABLE_OTA && OTA_TRANSPORT_DEFLATE_RX) || OTA_TRANSPORT_DEFLATE_TEST
