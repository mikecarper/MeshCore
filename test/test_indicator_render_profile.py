...
'''
    ...
    def test_indicator_setup_page_uses_large_reflow_even_after_canvas_fallback(self):
        main = MAIN.read_text(encoding="utf-8")
        start = main.index("static bool hasIndicatorSetupTypography()")
        end = main.index("static bool saveCompanionWiFi", start)
        setup = main[start:end]
        predicate = setup[: setup.index("static int drawIndicatorSetupValue")]
        self.assertIn("#if defined(INDICATOR_TRANSPORT_RENDER_PROFILE)", predicate)
        self.assertIn("return true;", predicate)
        self.assertNotIn("renderWidth()", predicate)
        self.assertNotIn("renderHeight()", predicate)
        self.assertIn("renderIndicatorSetupDisplay", setup)
        self.assertIn('setCompactText(true);', setup)
        self.assertIn('setTextSize(4);', setup)
        self.assertIn('35, "JOIN"', setup)
        self.assertIn('drawIndicatorSetupValue(70, wifi_name);', setup)
        self.assertIn('strncmp(value, "MeshCore-Setup-", 15) == 0', setup)
        self.assertIn('y, "MeshCore-"', setup)
        self.assertIn('y + 17, value + 9', setup)
        self.assertIn('getTextWidth(value) > max_width * 2', setup)
        self.assertIn('line_height = 9;', setup)
        self.assertIn('105, "OPEN"', setup)
        self.assertIn('108, "WAIT"', setup)
        self.assertIn('drawIndicatorSetupAddress(140, address);', setup)
        self.assertIn('setCompactText(false);', setup)
