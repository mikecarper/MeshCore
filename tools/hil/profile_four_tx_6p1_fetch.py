"""Retrieve 6.1-symbol experiment records without changing earlier captures."""
import base64
import hashlib
import json
...
p=subprocess.run(['pwsh','-NoProfile','-File',r'C:\git\Adafruit_nRF52_Bootloader_OTAFIX\.tmp_mercermesh_bridge.ps1',
                  '-ScriptPath',str(Path(__file__).with_name('profile_four_tx_6p1_export.py')),'-DeadlineSeconds','60'],
                 capture_output=True,text=True,cwd=root,timeout=65,check=True)
prefix='FOUR_TX_6P1_CAPTURE '
...
targets={name:root/'tools/hil'/('profile_four_tx_sf10_6p1_'+name) for name in names}
if any(path.exists() for path in targets.values()):raise RuntimeError('Preserve existing local captures')
for name,record in records.items():
    json.loads(record) # Validate before saving exact UTF-8 bytes from the Pi.
